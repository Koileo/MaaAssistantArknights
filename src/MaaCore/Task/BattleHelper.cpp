...
bool asst::BattleHelper::check_skip_plot_button(const cv::Mat& reusable)
{
    ...
    confirm_analyzer.set_task_info("SkipThePreBattlePlotConfirm");
    if (confirm_analyzer.analyze().has_value()) {
    ...
    if (ret) {
        ProcessTask(this_task(), { "SkipThePreBattlePlot" }).set_retry_times(3).run();
        return true;
    }
    ...
    ProcessTask(this_task(), { "SkipThePreBattlePlotConfirm" }).set_retry_times(5).run();
    return true;
}
...
bool asst::BattleHelper::check_in_battle(const cv::Mat& reusable, bool weak)
{
    ...
    if (weak) {
        ...
        if (result.has_value() && result->pause_button) {
            m_in_battle = true;
        }
        else {
            // 没有暂停按钮（或者 analyzer 返回 nullopt 无法识别战斗UI），优先检查是否处于关卡内剧情或头像对话框状态
